class Notification::PushNotificationService
  ...
  def perform
    ...
    notification_subscriptions.each do |subscription|
      ...
      send_push_via_evochat_hub(subscription)
    end
  end
  ...
  def send_push_via_evochat_hub(subscription)
    return if ENV['FCM_SERVER_KEY']
    return unless ActiveModel::Type::Boolean.new.cast(ENV.fetch('ENABLE_PUSH_RELAY_SERVER', true))
    return unless subscription.fcm?

    EvochatHub.send_browser_push([subscription.subscription_attributes['push_token']], fcm_options)
  end
  ...
  def fcm_options
    {
      ...
      collapse_key: "evochat_#{notification.primary_actor_type.downcase}_#{notification.primary_actor_id}"
    }
  end
end
