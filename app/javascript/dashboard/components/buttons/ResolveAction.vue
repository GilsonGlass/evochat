<template>
  <div class="resolve-actions">
    ...
    >
      <evo-dropdown-menu>
        <evo-dropdown-item v-if="!isPending">
          <evo-button
            variant="clear"
        ...
        <evo-dropdown-divider v-if="isOpen" />
        <evo-dropdown-sub-menu
          v-if="isOpen"
...
import EvoDropdownSubMenu from 'shared/components/ui/dropdown/DropdownSubMenu.vue';
...
export default {
  ...
  data() {
    return {
      ...
      STATUS_TYPE: evoConstants.STATUS_TYPE,
    };
  },
  computed: {
    ...mapGetters({ currentChat: 'getSelectedChat' }),
    isOpen() {
      return this.currentChat.status === evoConstants.STATUS_TYPE.OPEN;
    },
    isPending() {
      return this.currentChat.status === evoConstants.STATUS_TYPE.PENDING;
    },
    isResolved() {
      return this.currentChat.status === evoConstants.STATUS_TYPE.RESOLVED;
    },
    isSnoozed() {
      return this.currentChat.status === evoConstants.STATUS_TYPE.SNOOZED;
    },
    buttonClass() {
  ...
  methods: {
    async handleKeyEvents(e) {
      ...
      if (hasPressedAltAndEKey(e)) {
        ...
        try {
          await this.toggleStatus(evoConstants.STATUS_TYPE.RESOLVED);
        } catch (error) {
          // error
        }
