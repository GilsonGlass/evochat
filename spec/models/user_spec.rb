...
RSpec.describe User do
  ...
  describe 'hmac_identifier' do
    it 'return nil if EVOCHAT_INBOX_HMAC_KEY is not set' do
      expect(user.hmac_identifier).to eq('')
    end

    it 'return value if EVOCHAT_INBOX_HMAC_KEY is set' do
      ConfigLoader.new.process
      i = InstallationConfig.find_by(name: 'EVOCHAT_INBOX_HMAC_KEY')
