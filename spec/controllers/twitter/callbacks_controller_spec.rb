...
RSpec.describe 'Twitter::CallbacksController', type: :request do
  ...
  before do
    ...
    allow(raw_response).to receive(:body).and_return('oauth_token=1&oauth_token_secret=1&user_id=100&screen_name=evochat')
  ...
  describe 'GET /twitter/callback' do
    it 'creates inboxes if subscription is successful' do
      ...
      expect(account.twitter_profiles.last.inbox.name).to eq 'evochat'
