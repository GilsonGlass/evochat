...
RSpec.describe '/api/v1/widget/conversations/toggle_typing', type: :request do
  ...
  describe 'POST /api/v1/widget/conversations' do
    it 'creates a conversation' do
      post '/api/v1/widget/conversations',
           ...
           params: {
             ...
             contact: {
               name: 'contact-name',
               email: 'contact-email@evochat.com',
      ...
      expect(json_response['contact']['email']).to eq 'contact-email@evochat.com'
    ...
    it 'does not update the name if the contact already exist' do
      existing_contact = create(:contact, account: account, email: 'contact-email@evochat.com')
  ...
  describe 'POST /api/v1/widget/conversations/set_custom_attributes' do
    let(:params) { { website_token: web_widget.website_token, custom_attributes: { 'product_name': 'Evochat' } } }
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should have "product_name" key with value "Evochat"
        expect(conversation.custom_attributes).to include('product_name' => 'Evochat')
      end
    end
  end
  ...
  describe 'POST /api/v1/widget/conversations/destroy_custom_attributes' do
    ...
    context 'with correct website token' do
      it 'sets the values when provided' do
        ...
        conversation.custom_attributes = { 'product_name': 'Evochat' }
        conversation.save!
        expect(conversation.custom_attributes).to include('product_name' => 'Evochat')
        ...
        expect(response).to have_http_status(:success)
        conversation.reload
        # conversation custom attributes should not have "product_name" key with value "Evochat"
        expect(conversation.custom_attributes).not_to include('product_name' => 'Evochat')
      end
    end
  end
end
