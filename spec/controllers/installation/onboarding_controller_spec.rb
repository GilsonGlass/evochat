...
RSpec.describe 'Installation::Onboarding API', type: :request do
  ...
  describe 'GET /installation/onboarding' do
    context 'when EVOCHAT_INSTALLATION_ONBOARDING redis key is not set' do
      it 'redirects back' do
        expect(::Redis::Alfred.get(::Redis::Alfred::EVOCHAT_INSTALLATION_ONBOARDING)).to be_nil
        get '/installation/onboarding'
        expect(response).to have_http_status(:redirect)
      end
    end

    context 'when EVOCHAT_INSTALLATION_ONBOARDING redis key is set' do
      it 'returns onboarding page' do
        ::Redis::Alfred.set(::Redis::Alfred::EVOCHAT_INSTALLATION_ONBOARDING, true)
        get '/installation/onboarding'
        expect(response).to have_http_status(:success)
        ::Redis::Alfred.delete(::Redis::Alfred::EVOCHAT_INSTALLATION_ONBOARDING)
      end
    end
  end
  ...
  describe 'POST /installation/onboarding' do
    ...
    before do
      ...
      allow(EvochatHub).to receive(:register_instance).and_return(true)
      ::Redis::Alfred.set(::Redis::Alfred::EVOCHAT_INSTALLATION_ONBOARDING, true)
    end
    ...
    context 'when onboarding successfull' do
      ...
      it 'will not call register instance when checkboxes are unchecked' do
        post '/installation/onboarding', params: { user: {} }
        expect(EvochatHub).not_to have_received(:register_instance)
      end
      ...
      it 'will call register instance when checkboxes are checked' do
        ...
        expect(EvochatHub).to have_received(:register_instance)
      end
    end
    ...
    context 'when onboarding is not successfull' do
      it 'does not deletes the redis key' do
        ...
        post '/installation/onboarding', params: { user: {} }
        expect(::Redis::Alfred.get(::Redis::Alfred::EVOCHAT_INSTALLATION_ONBOARDING)).not_to be_nil
      end
    end
  end
end
