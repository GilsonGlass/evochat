...
RSpec.describe SupportMailbox, type: :mailbox do
  ...
  describe 'when a evochat notification email is received' do
    let(:account) { create(:account) }
    let!(:channel_email) { create(:channel_email, email: 'sojan@evochat.com', account: account) }
  ...
  describe 'add mail as a new ticket in the email inbox' do
    ...
    describe 'covers email address format' do
      ...
      it 'creates contact with proper email address' do
        expect(support_in_reply_to_mail.mail['reply_to'].try(:value)).to eq('Sony Mathew <sony@evochat.com>')
        expect(conversation.contact.email).to eq('sony@evochat.com')
      end
    end
    ...
    describe 'group email sender' do
      ...
      before do
        ...
        channel_email.email = 'support@evochat.com'
        channel_email.save!
      end
    ...
    describe 'when mail has in reply to email' do
      ...
      it 'append message to email conversation with same in reply to' do
        ...
        reply_mail_without_uuid.mail['In-Reply-To'] = 'conversation/6bdc3f4d-0bec-4515-a284-5d916fdde489/messages/123'
        reply_mail_without_uuid.mail['Message-Id'] = '0CB459E0-0336-41DA-BC88-E6E28C697SFC@evochat.com'
      ...
      it 'Considers raw html mail body' do
        ...
        expect(conversation.messages.last.content_attributes['email']['html_content']['reply']).to include(
          <<~BODY.chomp
            ...
            Evochat | CS team | [C](https://d33wubrfki0l68.cloudfront.net/973467c532160fd8b940300a43fa85fa2d060307/dc9a0/static/brand-73f58cdefae282ae74cebfa74c1d7003.svg)
