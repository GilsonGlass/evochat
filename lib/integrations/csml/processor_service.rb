class Integrations::Csml::ProcessorService < Integrations::BotProcessorService
  ...
  def client_params(session_id)
    {
      bot_id: "evochat-bot-#{conversation.inbox.id}",
      channel_id: "evochat-bot-inbox-#{conversation.inbox.id}",
      user_id: session_id
    }
  end
  ...
  def bot_payload
    {
      id: "evochat-csml-bot-#{agent_bot.id}",
      name: "evochat-csml-bot-#{agent_bot.id}",
      default_flow: 'evochat_bot_flow',
      flows: [
        {
          id: "evochat-csml-bot-flow-#{agent_bot.id}-inbox-#{conversation.inbox.id}",
          name: 'evochat_bot_flow',
