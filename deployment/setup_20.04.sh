...
function exit_handler() {
  if [ "$?" -ne 0 ] && [ "$u" == "n" ]; then
   echo -en "\nSome error has occured. Check '/var/log/evochat-setup.log' for details.\n"
   exit 1
  fi
}
...
function create_cw_user() {
  if ! id -u "evochat"; then
    adduser --disabled-login --gecos "" evochat
  fi
}
...
function save_pgpass() {
  mkdir -p /opt/evochat/config
  file="/opt/evochat/config/.pg_pass"
  if ! test -f "$file"; then
    echo $pg_pass > /opt/evochat/config/.pg_pass
  fi
}
...
##############################################################################
# Configure postgres to create evochat db user.
...
function configure_db() {
  ...
  sudo -i -u postgres psql << EOF
    ...
    CREATE USER evochat CREATEDB;
    ALTER USER evochat PASSWORD :'pass';
    ALTER ROLE evochat SUPERUSER;
...
function setup_evochat() {
  ...
  sed -i -e '/POSTGRES_USERNAME/ s/=.*/=evochat/' .env
...
function run_db_migrations(){
  sudo -i -u evochat << EOF
  cd evochat
  RAILS_ENV=production POSTGRES_STATEMENT_TIMEOUT=600s bundle exec rails db:evochat_prepare
EOF
}
...
function configure_systemd_services() {
  cp /home/evochat/evochat/deployment/evochat-web.1.service /etc/systemd/system/evochat-web.1.service
  cp /home/evochat/evochat/deployment/evochat-worker.1.service /etc/systemd/system/evochat-worker.1.service
  cp /home/evochat/evochat/deployment/evochat.target /etc/systemd/system/evochat.target

  cp /home/evochat/evochat/deployment/evochat /etc/sudoers.d/evochat
  cp /home/evochat/evochat/deployment/setup_20.04.sh /usr/local/bin/cwctl
  chmod +x /usr/local/bin/cwctl

  systemctl enable evochat.target
  systemctl start evochat.target
}
...
function setup_ssl() {
  ...
  wget https://raw.githubusercontent.com/evochat/evochat/develop/deployment/nginx_evochat.conf
  ...
  ln -s /etc/nginx/sites-available/nginx_evochat.conf /etc/nginx/sites-enabled/nginx_evochat.conf
  systemctl restart nginx
  sudo -i -u evochat << EOF
  cd evochat
  sed -i "s/http:\/\/0.0.0.0:3000/https:\/\/$domain_name/g" .env
EOF
  systemctl restart evochat.target
}
...
function setup_logging() {
  touch /var/log/evochat-setup.log
  LOG_FILE="/var/log/evochat-setup.log"
}
...
'tail -f /var/log/evochat-setup.log'.
...
function get_console() {
  sudo -i -u evochat bash -c " cd evochat && RAILS_ENV=production bundle exec rails c"
}
...
Report bugs at https://github.com/GilsonGlass/evochat/issues
...
function get_logs() {
  if [ "$SERVICE" == "worker" ]; then
    journalctl -u evochat-worker.1.service -f
  fi
  if [ "$SERVICE" == "web" ]; then
    journalctl -u evochat-web.1.service -f
  fi
}
...
function restart() {
  systemctl restart evochat.target
  systemctl status evochat.target
}
